...
public class MiniMaxPlayer extends Player {
    ...
    /**
     * @brief Contador de una serie de fichas para realizar la jugada
     * @param est
     * @param jugadorMov
     * @param longitud para saber la serie
     * @return numero de la serie de fichas
     */ 
    public int contadorFichas(InfoMiniMax est, int jugadorMov, int longitud){
        int contHorizontal, contVertical, contDiagonalIzq, contDiagonalDer, contFichas = 0;
        for (int i=0; i<est.tablero.length; i++) {
            for (int j=0; j<est.tablero[0].length; j++) {
                if (est.tablero[i][j]==jugadorMov) {
                    contVertical = contVertical(i, j, jugadorMov, est.tablero);
                    if (contVertical == longitud) 
                        contFichas++;
                    contHorizontal = contarHorizontal(i, jugadorMov, est.tablero);
                    if (contHorizontal == longitud) 
                        contFichas++;
                    contDiagonalDer = contDiagDer(i, j, jugadorMov, est.tablero);
                    if (contDiagonalDer == longitud) 
                        contFichas++;
                    contDiagonalIzq = contDiagIzq(i, j, jugadorMov, est.tablero);
                    if (contDiagonalIzq == longitud) 
                        contFichas++;  
                }
            }
        }
    ...
    private int contVertical(int fila, int col, int ficha, int[][]m) {
        int cont = 0;
    ...
    private int contarHorizontal(int fila, int ficha, int[][]m) {
        int cont = 0;
    ...
    private int contDiagDer(int fil, int col, int ficha, int[][]m) {
        int filDiag = 0, colDiag = 0;
        int j = col;
        int cont = 0;
        for (int i = fil; i < m.length && j >= 0; i++) {
            filDiag = fil;
            colDiag = j;
            j--;
        }
        j = colDiag;
        for (int i = filDiag; i >= 0 && j < m[0].length; i--)
            if (m[i][j++] == ficha)
                cont++;
        return cont;
    }
    
    private int contDiagIzq(int fil, int col, int ficha, int[][]m) {
        int filDiag=0, colDiag=0;
        int j = col;
        int cont = 0;
        for (int i=fil; i<m.length && j<m[0].length; i++) {
           filDiag = fil;
           colDiag = j;
           j++;
        }
        j = colDiag;
        for (int i = filDiag; i>=0 && j>=0; i--)
            if (m[i][j--] == ficha)
                cont++;
        return cont;
    }
            ...
            for (int j = 0; j <est.tablero[0].length; j++) { //Bucle para obtener las 7 posibles tiradas   
                ...
                for (int i=est.tablero.length-1; i>=0 && !sePuedeMeter; i--) {
                    ...
                    if (sePuedeMeter) {
                        ...
                        //Metemos la ficha para la posible tirada
    ...
    private int MiniMaxMAX(InfoMiniMax est, int jugador, int profundidad){
        ...
        if (contadorFichas(est, jugador, 4)!=0 || profundidad == PROFUNDIDAD+1 ||!sePuedeMeter)
            ...
            return (contadorFichas(est, jugador, 2) + contadorFichas(est, jugador, 3) - (contadorFichas(est, jugador * (-1), 2) + contadorFichas(est, jugador * (-1), 3)));
        else {
